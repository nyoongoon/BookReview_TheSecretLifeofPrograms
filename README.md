...
## 함수(프로시저, 서브루틴)
- 코드의 중복을 피하기 위해서 함수를 사용한다. 
- 함수 코드가 작동하는 원리
          
            함수를 호출하는 부분에서 함수를 실행하고 다시 원래 자리로 돌아올 방법이 필요.
            원래 자리로 돌아오기 위해서는 어디서 함수로 들어갔는지를 기억해야함.
            함수로 들어간 위치는 바로 프로그램 카운터(PC)의 값.
...
## 스택
- 함수를 호출하기 위해서는 돌아올 원래 자리의 주소가 필요.
- 함수a 안에서 함수b를 호출할 경우, b를 호출하고 돌아갈 주소와, a를 호출하고 돌아갈 주소가 필요하다. 
            
         따라서, 함수를 호출할 때는 반환 주소를 접시에 넣어서 집시 더미 맨 뒤에 넣는다. 
         함수 호출에서 돌아올 때는 접시 더미 맨 위의 접시를 보고 반환주소를 결정한 다음, 접시를 제거한다.
         이러한 구조를 스택이라고 한다.
- 스택에 물건을 쌓는데 더이상 들어갈 공간이 없으면 **스택 오버플로**
- 빈 스택에서 물건을 가져오라고 하는 경우를 **스택 언더플로**라한다.
- 함수에서 지역 변수도 그냥 덮어 쓰면 안되기 때문에 지역 변수도 스택에 저장해야한다. 
- 이렇게 함수가 호출될 때마다 스택에 저장되는 데이터의 모음을 **스택 프레임**이라고 한다. 
